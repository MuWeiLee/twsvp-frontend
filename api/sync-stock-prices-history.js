...
export default async function handler(req, res) {
  ...
  const endDate = `${params.end_date || params.endDate || getTaipeiDateString()}`;
  ...
  const dataset =
    params.dataset || process.env.STOCK_PRICE_HISTORY_DATASET || "TaiwanStockPrice";
  const maxRuntimeMs = resolveMaxRuntimeMs(
    params.max_runtime_ms || process.env.STOCK_PRICE_HISTORY_MAX_RUNTIME_MS,
    290000
  );
  const requestOverheadMs = resolveRequestOverheadMs(
    params.request_overhead_ms || process.env.STOCK_PRICE_HISTORY_REQUEST_OVERHEAD_MS,
    1000
  );
